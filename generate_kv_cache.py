...
import argparse
from model_kv_cache import Decoder
import torch
import json
...
class Generate:
    def __init__(self, checkpoint_path, temperature=1, decoding="greey", beam_width=5, kv_cache=False):
    ...
    def generate(self, perplexity, num_samples=10):
        ...
        experiment_directory = "kv_cache" if self.kv_cache else "no_kv_cache"
        os.makedirs(os.path.join(self.config["training"]["output_dir"], experiment_directory), exist_ok=True)
        with open(
            os.path.join(self.config["training"]["output_dir"], experiment_directory, "metrics.json"), "w"
        ) as f:
            json.dump(
                {
        ...
        with open(
            os.path.join(self.config["training"]["output_dir"], experiment_directory, "responses.json"), "w"
        ) as f:
...
if __name__ == "__main__":
    ...
    generate = Generate(args.checkpoint_path, args.temperature, args.decoding, args.beam_width, args.kv_cache)
    generate.generate(0, num_samples=args.num_samples)
