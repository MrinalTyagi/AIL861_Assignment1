...
class KVCache:
    ...
    def is_empty(self):
        if self.cache["key"] is None or self.cache["value"] is None:
            return True
        return False
...
def attention_with_kv_cache(query, key, value):
    d_k = torch.tensor(query.shape[-1], device=query.device)
    attn_score = query @ key.transpose(-1, -2) / torch.sqrt(d_k)
    softmax_score = nn.functional.softmax(attn_score, dim=-1)
    softmax_score = softmax_score.masked_fill(torch.isnan(softmax_score), 0)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.transformer_layers = nn.ModuleList(
            [
                TransformerBlock(self.num_heads, self.input_dim, self.hidden_dim, self.use_kv_cache)
...
class TransformerBlock(nn.Module):
    def __init__(self, num_heads, input_dim, hidden_dim, use_kv_cache=False):
        ...
        self.mhsa = MultiHeadSelfAttention(
            self.num_heads, self.input_dim, self.hidden_dim, self.use_kv_cache
        )
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, inputs, attn_mask=None):
        ...
        if self.kv_cache is None:
            ...
            current_token = inputs[:, -1, :].unsqueeze(1)
            batch_size, seq_length, hidden_dim = current_token.shape
            q = self.Wq(current_token)
            if self.kv_cache.is_empty():
                new_k = self.Wk(inputs)
                new_v = self.Wv(inputs)
            else:
                new_k = self.Wk(current_token)
                new_v = self.Wv(current_token)
            ...
            k = k.view(batch_size, inputs.shape[1], self.num_heads, self.hidden_dim // self.num_heads).transpose(1, 2)
            v = v.view(batch_size, inputs.shape[1], self.num_heads, self.hidden_dim // self.num_heads).transpose(1, 2)
            attn_output = attention_with_kv_cache(q, k, v).transpose(1, 2)
            attn_output = attn_output.contiguous().view(batch_size, 1, -1)
