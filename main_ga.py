    ...
    def train(self):
        ...
        for epoch in range(1, self.config["training"]["epochs"] + 1):
            ...
            for data in training_progress_bar:
                ...
                train_loss = train_loss / self.config["training"]["accum_step"]
                train_loss.backward()
                ...
                if (self.training_steps + 1) % self.config["training"][
                    ...
                    self.writer.add_scalar(
                        "train/loss_per_step", train_loss.item() * self.config["training"]["accum_step"], self.training_steps
                    )
                    self.writer.add_scalar(
                        "train/lr",
                    ...
                    wandb.log(
                        {
                            "train/loss_per_step": train_loss.item() * self.config["training"]["accum_step"],
                            "train/lr": self.optimizer.param_groups[0]["lr"],
                        },
                        step=self.training_steps,
                    )
                self.training_steps += 1
                training_progress_bar.set_postfix(loss=train_loss.item() * self.config["training"]["accum_step"])
